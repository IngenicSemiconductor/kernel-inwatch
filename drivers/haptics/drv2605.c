...
static int drv2605_set_waveform_sequence(struct drv2605_data *pDrv2605data, unsigned char* seq, unsigned int size)
{
    return drv2605_bulk_write(pDrv2605data, WAVEFORM_SEQUENCER_REG,
            (size > WAVEFORM_SEQUENCER_MAX) ? WAVEFORM_SEQUENCER_MAX : size,
            seq);
}
    ...
    if (dev_mode == DEV_IDLE) {
        ...
        if ((work_mode != pDrv2605data->work_mode)
            ...
            if ((pDrv2605data->work_mode == WORK_VIBRATOR)
                ...
                schedule_timeout_interruptible(
                        msecs_to_jiffies(STANDBY_WAKE_DELAY));
            }
        }
    }
}
...
static void setAudioHapticsEnabled(struct drv2605_data *pDrv2605data, int enable)
{
    if (enable) {
        if (pDrv2605data->work_mode != WORK_AUDIO2HAPTIC) {
            ...
            drv2605_set_bits(pDrv2605data, Control1_REG,
                    Control1_REG_AC_COUPLE_MASK, AC_COUPLE_ENABLED);

            drv2605_set_bits(pDrv2605data, Control3_REG,
                    Control3_REG_PWMANALOG_MASK, INPUT_ANALOG);
        ...
        if (pDrv2605data->work_mode == WORK_AUDIO2HAPTIC) {
            ...
            drv2605_set_bits(pDrv2605data, Control1_REG,
                    Control1_REG_AC_COUPLE_MASK, AC_COUPLE_DISABLED);

            drv2605_set_bits(pDrv2605data, Control3_REG,
                    Control3_REG_PWMANALOG_MASK, INPUT_PWM);
    ...
    if (pDrv2605data->work_mode == WORK_PATTERN_RTP_ON) {
        ...
        if (pDrv2605data->repeat_times == 0) {
            ...
            hrtimer_start(&pDrv2605data->timer,
                    ns_to_ktime(
                            (u64) pDrv2605data->silience_time * NSEC_PER_MSEC),
                    HRTIMER_MODE_REL);
        }
        ...
        hrtimer_start(&pDrv2605data->timer,
                ns_to_ktime((u64) pDrv2605data->vibration_time * NSEC_PER_MSEC),
                HRTIMER_MODE_REL);
    }
}
...
static void play_Seq_RTP(struct drv2605_data *pDrv2605data)
{
    if (pDrv2605data->RTPSeq.RTPindex < pDrv2605data->RTPSeq.RTPCounts) {
        int RTPTime =
                pDrv2605data->RTPSeq.RTPData[pDrv2605data->RTPSeq.RTPindex]
                        >> 8;
        int RTPVal = pDrv2605data->RTPSeq.RTPData[pDrv2605data->RTPSeq.RTPindex]
                & 0x00ff;
        ...
        hrtimer_start(&pDrv2605data->timer,
                ns_to_ktime((u64) RTPTime * NSEC_PER_MSEC), HRTIMER_MODE_REL);
    } else {
        drv2605_change_mode(pDrv2605data, WORK_IDLE, DEV_STANDBY);
        pDrv2605data->vibrator_is_playing = NO;
        switch_set_state(&pDrv2605data->sw_dev, SW_STATE_IDLE);
...
static ssize_t dev2605_write(struct file* filp, const char* buff, size_t len, loff_t* off)
{
    ...
    case HAPTIC_CMDID_PATTERN_RTP: {
        ...
        pDrv2605data->vibration_time = (int) ((((int) buff[2]) << 8)
                | (int) buff[1]);
        pDrv2605data->silience_time = (int) ((((int) buff[4]) << 8)
                | (int) buff[3]);
        strength = buff[5];
        ...
        if (pDrv2605data->vibration_time > 0) {
            ...
            hrtimer_start(&pDrv2605data->timer,
                    ns_to_ktime(
                            (u64) pDrv2605data->vibration_time * NSEC_PER_MSEC),
                    HRTIMER_MODE_REL);
        }
        break;
    }
    ...
    case HAPTIC_CMDID_RTP_SEQUENCE: {
        ...
        if (((len - 1) % 2) == 0) {
            ...
            if ((pDrv2605data->RTPSeq.RTPCounts <= MAX_RTP_SEQ)
                ...
                if (copy_from_user(pDrv2605data->RTPSeq.RTPData, &buff[1],
                        pDrv2605data->RTPSeq.RTPCounts * 2) != 0) {
    ...
    case HAPTIC_CMDID_REG_READ: {
        if (len == 2) {
            ...
            pDrv2605data->ReadLen =
                    (buff[2] > MAX_READ_BYTES) ? MAX_READ_BYTES : buff[2];
            drv2605_bulk_read(pDrv2605data, buff[1], pDrv2605data->ReadLen,
    ...
    case HAPTIC_CMDID_REG_WRITE: {
        if ((len - 1) == 2) {
            ...
            unsigned char *data = (unsigned char *) kzalloc(len - 2,
                    GFP_KERNEL);
            if (data != NULL) {
...
static int Haptics_init(struct drv2605_data *pDrv2605data)
{
    ...
    reval = alloc_chrdev_region(&pDrv2605data->version, 0, 1,
            HAPTICS_DEVICE_NAME);
    if (reval < 0) {
    ...
    pDrv2605data->device = device_create(pDrv2605data->class, NULL,
            pDrv2605data->version, NULL, HAPTICS_DEVICE_NAME);
...
static void dev_init_platform_data(struct drv2605_data *pDrv2605data)
{
    ...
    if (pDrv2605data->OTP == 0) {
        if (actuator.rated_vol != 0) {
            drv2605_reg_write(pDrv2605data, RATED_VOLTAGE_REG,
                    actuator.rated_vol);
        } else {
        ...
        if (actuator.over_drive_vol != 0) {
            drv2605_reg_write(pDrv2605data, OVERDRIVE_CLAMP_VOLTAGE_REG,
                    actuator.over_drive_vol);
        } else {
            printk("%s, ERROR OverDriveVol ZERO\n", __FUNCTION__);
        }

        drv2605_set_bits(pDrv2605data, FEEDBACK_CONTROL_REG,
    ...
    if (actuator.device_type == LRA) {
        ...
        printk("%s, LRA = %d, DriveTime=0x%x\n", __FUNCTION__, actuator.LRAFreq,
                DriveTime);
    }

    drv2605_set_bits(pDrv2605data, Control2_REG, Control2_REG_BIDIR_INPUT_MASK,
            temp);
...
static int drv2605_probe(struct i2c_client* client, const struct i2c_device_id* id)
{
    ...
    pDrv2605data = devm_kzalloc(&client->dev, sizeof(struct drv2605_data),
            GFP_KERNEL);
    if (pDrv2605data == NULL) {
    ...
    if (pDrv2605data->PlatData.GpioTrigger) {
        err = gpio_request(pDrv2605data->PlatData.GpioTrigger,
    ...
    if (pDrv2605data->PlatData.GpioEnable) {
        err = gpio_request(pDrv2605data->PlatData.GpioEnable,
...
{
    .driver = {
        .name = HAPTICS_DEVICE_NAME, .owner = THIS_MODULE,
#if defined(CONFIG_PM)
        .pm = &drv2605_pm_ops,
#endif
    },
