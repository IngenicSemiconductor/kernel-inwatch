...
static int ricoh61x_rtc_alarm_is_enabled(struct device *dev,  uint8_t *enabled)
{
	...
	err = ricoh61x_read_regs(dev, rtc_ctrl1, 1, &reg_data);
	if (err) {
		dev_err(dev->parent, "read rtc_ctrl1 error 0x%d\n", err);
		*enabled = 0;
	} else {
		if (reg_data & 0x40)
			*enabled = 1;
...
static int ricoh61x_rtc_alarm_enable(struct device *dev, unsigned int enabled)
{
	...
	if (enabled) {
		...
		err = ricoh61x_read_regs(dev, rtc_ctrl1, 1, &reg_data);
		if (err < 0) {
			dev_err(dev->parent, "PMU: %s read rtc_ctrl1 error 0x%d\n", __func__, err);
			goto ERR;
		}
		reg_data |= 0x40;	/* set DALE */
		err = ricoh61x_write_regs(dev, rtc_ctrl1, 1, &reg_data);
		if (dev < 0)
			dev_err(dev->parent, "PMU: %s write rtc_ctrl1 error 0x%d\n", __func__, err);
	} else {
		rtc->irq_en = 0;
		err = ricoh61x_read_regs(dev, rtc_ctrl1, 1, &reg_data);
		if (err < 0) {
...
static int ricoh61x_rtc_read_alarm(struct device *dev, struct rtc_wkalrm *alrm)
{
	...
	if (err) {
		dev_err(dev->parent, "RTC: %s *** read rtc_alarm timer error 0x%d\n", __func__, err);
		return err;
	}

	err = ricoh61x_read_regs(dev, rtc_ctrl1, 1, &enabled_flag);
	if (err) {
		dev_err(dev->parent, "RTC: %s *** read rtc_enable flag error 0x%d\n", __func__, err);
		return err;
	}
...
static int ricoh61x_rtc_set_alarm(struct device *dev, struct rtc_wkalrm *alrm)
{
	...
	printk("alrm->enabled = %d,  %d, %d \n", alrm->enabled,
			alrm->time.tm_year, os_ref_year);
	if (alrm->enabled == 0)
		return 0;
...
static int ricoh61x_rtc_probe(struct platform_device *pdev)
{
	...
	if (rtc->irq != -1) {
		...
		if (err) {
			...
			printk("----------------------> rtc_irq enable when probe\n");
			if (err) {
	...
	ricoh61x_read_regs(&pdev->dev, rtc_ctrl1, 1, &buff[0]);
	ricoh61x_read_regs(&pdev->dev, rtc_ctrl2, 1, &buff[1]);
