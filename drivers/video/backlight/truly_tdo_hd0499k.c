...
static unsigned char truly_tdo_hd0499k_cmd_list[] = {
	0x15, 0xFF,0x00,
...
static void truly_tdo_hd0499k_panel_init(struct truly_tdo_hd0499k *lcd)
{
	...
	struct dsi_master_ops *ops = lcd_to_master_ops(lcd);
	struct dsi_device *dsi = lcd_to_master(lcd);
	array_size = ARRAY_SIZE(truly_tdo_hd0499k_cmd_list);
	ops->cmd_write(dsi, truly_tdo_hd0499k_cmd_list, array_size);
}
